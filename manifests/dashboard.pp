class githubsync::dashboard {
  ...
  cron { "update githubsync dashboard":
    ...
    command => "/usr/local/bin/githubsync-update-status.sh 2>&1 | logger -t githubsync-update-status; /usr/local/bin/githubsync-dashboard.rb > /var/local/run/githubsync/current-status.txt",
    user    => "githubsync",
    hour    => "*/6",
    minute  => ip_to_cron(),
